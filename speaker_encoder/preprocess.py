...
from speaker_encoder.config import librispeech_datasets
...
class DatasetLog:
    ...
    def finalize(self):
        ...
        for param_name, values in self.sample_data.items():
            ...
            self.write_line(
                "\t\tmean %.3f, median %.3f" % (np.mean(values), np.median(values))
            )
        self.write_line("-----")
...
def _preprocess_speaker_dirs(
    speaker_dirs, dataset_name, datasets_root, out_dir, extension, skip_existing, logger
):
    ...
    with ThreadPool(8) as pool:
        list(
            tqdm(
                pool.imap(preprocess_speaker, speaker_dirs),
                dataset_name,
                len(speaker_dirs),
                unit="speakers",
            )
...
def _preprocess_speaker_dirs_vox2(
    speaker_dirs, dataset_name, datasets_root, out_dir, extension, skip_existing, logger
):
...
def preprocess_voxceleb1(datasets_root: Path, out_dir: Path, skip_existing=False):
    ...
    # Preprocess all speakers
    _preprocess_speaker_dirs(
        speaker_dirs, dataset_name, datasets_root, out_dir, "wav", skip_existing, logger
    )
...
def preprocess_voxceleb2(datasets_root: Path, out_dir: Path, skip_existing=False):
    ...
    _preprocess_speaker_dirs_vox2(
        speaker_dirs, dataset_name, datasets_root, out_dir, "m4a", skip_existing, logger
    )
